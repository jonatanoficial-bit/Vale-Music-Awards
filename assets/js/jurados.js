...
import {
  collection, getDocs, doc, getDoc, setDoc, runTransaction, serverTimestamp, query, orderBy
...
function calcTotal100(scores){
  // 10 critérios x 0-10 => soma 0-100
...
function renderLogin(mount){
  mount.innerHTML = `
    <div style="display:grid;gap:10px">
      <b>Login de Jurado</b>
      <p style="color:var(--muted);margin:0">Acesso restrito do Vale Music Awards.</p>
      <input id="email" type="email" placeholder="email" style="padding:12px;border-radius:14px;border:1px solid rgba(255,255,255,.14);background:rgba(0,0,0,.30);color:var(--text)" />
      <input id="pass" type="password" placeholder="senha" style="padding:12px;border-radius:14px;border:1px solid rgba(255,255,255,.14);background:rgba(0,0,0,.30);color:var(--text)" />
      <button class="btn btn--gold" id="btnLogin">Entrar</button>
      <div style="color:var(--muted2);font-size:12px">Dica: crie 5 usuários de jurado no Firebase Authentication.</div>
    </div>
  `;
...
async function hasLockedScore(candidateId, judgeUid){
...
function scoreFormHTML(candidate){
  const crit = window.VMA.CRITERIA.map(c => `
    <div class="tile" style="padding:12px">
      <div style="display:flex;justify-content:space-between;gap:10px;align-items:center">
        <b>${c.label}</b>
        <span style="color:var(--muted2);font-size:12px">0 a 10</span>
      </div>
      <input data-k="${c.key}" type="range" min="0" max="10" step="1" value="8" style="width:100%;margin-top:10px">
  ...
  return `
    <div class="card" style="padding:16px;margin-top:12px">
      <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap">
        <img src="${candidate?.photo?.ucUrl || ""}" style="width:64px;height:64px;border-radius:18px;object-fit:cover;border:1px solid rgba(255,255,255,.10)"/>
        <div style="flex:1">
      ...
      <div style="margin-top:12px">
        ...
        <div style="margin-top:8px">
          <audio controls style="width:100%">
            <source src="${candidate?.audio?.ucUrl || ""}" type="audio/mpeg">
          </audio>
        </div>
      </div>
      ...
      <div style="margin-top:14px">
        ...
        <div style="color:var(--muted);margin-top:6px">Preencha os critérios (0–10). A nota final (0–100) é calculada automaticamente.</div>
        ...
        <textarea id="note" rows="3" placeholder="Feedback (opcional)" style="margin-top:10px;width:100%;padding:12px;border-radius:14px;border:1px solid rgba(255,255,255,.14);background:rgba(0,0,0,.30);color:var(--text)"></textarea>

        <button class="btn btn--gold btn--block" id="finalize" style="margin-top:12px">Finalizar avaliação (travada)</button>
...
function wireTotalCalc(root){
  ...
  function recompute(){
    ...
    for(const s of sliders){
      scores[s.dataset.k] = Number(s.value);
    }
...
async function finalizeScore(candidateId, judgeUid, scores, note){
  ...
  // 1) Cria score (se já existir, trava)
  await runTransaction(db, async (tx)=>{
    ...
    // 2) Atualiza média no candidato (avgScore100 / scoresCount)
...
async function renderPanel(mount, user){
  ...
  mount.innerHTML = `
    ...
    <div class="card" style="padding:14px;margin-top:12px">
      <b>Candidatos</b>
      <div style="color:var(--muted);margin-top:6px">Selecione um candidato para avaliar. Se já foi avaliado, ficará somente leitura.</div>
      <select id="selCand" style="margin-top:10px;width:100%;padding:12px;border-radius:14px;border:1px solid rgba(255,255,255,.14);background:rgba(0,0,0,.30);color:var(--text)">
        ${candidates.map(c=>`<option value="${c.id}">${c.id} • ${c.nomeArtistico || c.nome}</option>`).join("")}
      </select>
    </div>
  ...
  mount.querySelector("#logout").onclick = async ()=>{ await signOut(auth); };
  ...
  async function showCandidate(id){
    ...
    if(!cand){ area.innerHTML = ""; return; }

    const locked = await hasLockedScore(cand.id, user.uid);

    // Se já avaliado: exibe leitura
    if(locked){
      area.innerHTML = `
        <div class="card" style="padding:16px;margin-top:12px">
          <b>Avaliação finalizada (travada)</b>
          <div style="color:var(--muted);margin-top:6px">Você já finalizou esta avaliação e não pode editar.</div>

          <div style="display:flex;gap:12px;align-items:center;margin-top:12px;flex-wrap:wrap">
            <img src="${cand?.photo?.ucUrl || ""}" style="width:64px;height:64px;border-radius:18px;object-fit:cover;border:1px solid rgba(255,255,255,.10)"/>
            <div style="flex:1">
              <div style="font-weight:950;font-size:18px">${cand.nomeArtistico || cand.nome}</div>
              <div style="color:var(--muted)">${cand.id} • ${cand.genero} • ${cand.cidade}</div>
            </div>
          ...
          <div style="margin-top:12px">
            <b>Áudio</b>
            <div style="margin-top:8px">
              <audio controls style="width:100%"><source src="${cand?.audio?.ucUrl || ""}" type="audio/mpeg"></audio>
            </div>
          </div>
          ...
          <div style="margin-top:12px">
            ...
            <div style="margin-top:10px;display:grid;grid-template-columns:repeat(2,1fr);gap:10px">
              ${window.VMA.CRITERIA.map(cr=>`
                <div class="tile" style="padding:12px">
                  <b>${cr.label}</b>
                  <div style="margin-top:6px;color:var(--muted)">Nota: <b>${Number(locked?.scores?.[cr.key] ?? 0)}</b> / 10</div>
                </div>
              `).join("")}
            </div>
          </div>

          ${locked.note ? `<div style="margin-top:12px"><b>Feedback</b><div style="color:var(--muted);margin-top:6px">${locked.note}</div></div>` : ""}
    ...
    // Ainda não avaliado: formulário completo
    ...
    area.querySelector("#finalize").onclick = async ()=>{
      ...
      try{
        ...
        // Recarrega para mostrar travado
        await showCandidate(cand.id);
      }catch(e){
